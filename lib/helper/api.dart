...
class Api{
  Future<dynamic> get ({required String url, @required String? token})async{
    ...
    if(response.statusCode == 200){
      ...
      throw Exception(
        'There is a probblem with the status code ${response.statusCode}',
      );
    }
  }
  ...
  Future <dynamic> post ({required String url, @required dynamic body, @required String? token}) async{
     ...
     http.Response response = await http.post(Uri.parse(url),
          ...
          if(response.statusCode == 200){
            ...
            throw Exception('There is a problem with the status code ${response.statusCode} with body ${jsonDecode(response.body)}');
          }
  }
